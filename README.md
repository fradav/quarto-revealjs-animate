...
A Revealjs plugin extension for animating SVG, entirely based on [@rajgoel](https://github.com/rajgoel/reveal.js-plugins/) work. Changes have been made to make it work with Quarto with YAML code blocks instead of JSON.
...
An animation can be included in a slide by adding a YAML code block with `animate` class. Furthermore, the filename of an SVG to be loaded can be provided by an attribute `src="drawing.svg"`. Alternatively, a `svg` element can be manually placed within the element. The animation is provided by the YAML string within the code block. The following example shows the basic structure:

`````markdown
```yaml { .animate src="drawing.svg" }
setup:
  - ...
animation:
  - ...
...
- `element` (optional): The selector for any element(s) within the SVG on which the `modifier` is executed with the given `parameters`. If multiple elements match the selector, the modifier is executed sequentially to all elements matched. If no `element` is provided, the `modifier` is executed on the SVG element.
